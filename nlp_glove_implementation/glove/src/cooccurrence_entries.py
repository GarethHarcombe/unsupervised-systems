        ...
        for partition_id in tqdm(range(len(split_points) - 1)):
            ...
            for i in tqdm(range(len(self.vectorized_corpus))):
                ...
                for j in range(context_lower, context_upper):
                    ...
                    df = (window_size + 1 - abs(i - j)) / window_size
                    ff = lambda w: (threshold / self.vectorizer.vocab[w]) ** 0.5 if self.vectorizer.vocab[w] > threshold else 1

                    cooccurr_counts[(self.vectorized_corpus[i], self.vectorized_corpus[j])] += 1 / (df * ff(self.vectorized_corpus[i]) * ff(self.vectorized_corpus[j]))
